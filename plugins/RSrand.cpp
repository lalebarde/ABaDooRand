...
RS_rand::RS_rand(ABaDooRand* aAbadorand, const rng_type aRNGtype, const uint64_t aSeed, const size_t aReseedPeriodMax, char* aArgs)
...
uint32_t RS_rand::raw_draw_uniform_int32() {
    return rand();
}

uint64_t RS_rand::raw_draw_uniform_int64() {
    return ((uint64_t) rand() << 32) | rand();
}

void RS_rand::raw_add_entropy(uint64_t aEntropy) {
    srand(aEntropy);
}

void RS_rand::raw_seed(uint64_t aSeed) {
    srand(aSeed);
}
